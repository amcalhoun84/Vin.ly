...
namespace Vin.ly
{
    [Activity(Label = "Find Wine by Region")]
    public class WineRegion : Activity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.WineRegion);
            ...
            button.Click += async (IntentSender, e) =>
            {
                try
                {
                    string url = "http://52.27.116.225:8000/api/user/" + _userName + "/wine/region/" + wineSearch.Text;
                    ...
                    {
                        if (json[i]["region"] == wineSearch.Text && json[i]["username"] == _userName)
                        {
                    ...
                    if (trueCount <= 0)
                    {
                        Error.Text = "No Wine Matching That Region Found.";
                    }
